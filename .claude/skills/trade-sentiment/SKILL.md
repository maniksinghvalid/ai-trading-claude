...
`plan/portfolio-routine-and-vector-memory.md`, SENTIMENT emits
`sentiment_score`, `signal`, `grade`, `price_at_analysis` (no `price_target`
or `stop_loss` — sentiment doesn't set entry/exit levels). Signal and grade
are **UPPERCASE** and derived from `sentiment_score` via the shared 6-band
...
signal: <STRONG BUY|BUY|HOLD|NEUTRAL|CAUTION|AVOID>   # derived from sentiment_score
grade: <A+|A|B|C|D|F>                                   # derived from sentiment_score
