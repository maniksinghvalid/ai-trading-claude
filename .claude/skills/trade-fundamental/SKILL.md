...
`plan/portfolio-routine-and-vector-memory.md`, FUNDAMENTAL emits
`fundamental_score`, `signal`, `grade`, `price_at_analysis` (no `price_target`
or `stop_loss` — fundamentals don't set entry/exit levels). Signal and grade
are **UPPERCASE** and derived from `fundamental_score` via the shared 6-band
table (`scripts/trade_scoring.py`: 85+/A+/STRONG BUY · 70-84/A/BUY ·
55-69/B/HOLD · 40-54/C/NEUTRAL · 25-39/D/CAUTION · 0-24/F/AVOID). Use the exact
enum values (STRONG BUY|BUY|HOLD|NEUTRAL|CAUTION|AVOID and A+|A|B|C|D|F).
...
signal: <STRONG BUY|BUY|HOLD|NEUTRAL|CAUTION|AVOID>   # derived from fundamental_score
grade: <A+|A|B|C|D|F>                                   # derived from fundamental_score
