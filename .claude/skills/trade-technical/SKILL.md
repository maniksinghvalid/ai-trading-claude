...
`plan/portfolio-routine-and-vector-memory.md`, TECHNICAL emits
`technical_score`, `signal`, `grade`, `price_at_analysis`, `price_target`,
`stop_loss` (no other scores). Signal and grade are **UPPERCASE** and derived
from `technical_score` via the shared 6-band table (`scripts/trade_scoring.py`:
...
signal: <STRONG BUY|BUY|HOLD|NEUTRAL|CAUTION|AVOID>   # derived from technical_score
grade: <A+|A|B|C|D|F>                                   # derived from technical_score
price_at_analysis: <float, USD>
price_target: <float, USD — Target 1 from the Trading Setup table>
stop_loss: <float, USD — Stop Loss from the Trading Setup table>
---
