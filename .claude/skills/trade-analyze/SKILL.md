...
Write the file `TRADE-ANALYSIS-<TICKER>.md` to the current working directory.
...
table in `plan/portfolio-routine-and-vector-memory.md`, ANALYSIS is the only
report_type that emits `composite_score` plus ALL five dimension scores
(technical/fundamental/sentiment/risk/thesis) plus full pricing + catalyst
fields. `risk_score` is **INVERTED — higher = lower risk** (so it composes
correctly into the weighted Trade Score; preserve this convention from the
per-dim Risk Assessment agent). Signal and grade are **UPPERCASE** and derived
from `composite_score` via the shared 6-band table (`scripts/trade_scoring.py`:
85+/A+/STRONG BUY · 70-84/A/BUY · 55-69/B/HOLD · 40-54/C/NEUTRAL ·
25-39/D/CAUTION · 0-24/F/AVOID). Use the exact enum values
...
composite_score: <int 0-100, from the weighted formula above>
technical_score: <int 0-100, from Agent 1>
fundamental_score: <int 0-100, from Agent 2>
sentiment_score: <int 0-100, from Agent 3>
risk_score: <int 0-100, from Agent 4 — INVERTED, higher = safer>
...
nearest_catalyst_date: <YYYY-MM-DD — earliest catalyst from the Catalyst Calendar>
---
...
**Step 4 — Ingest into Pinecone memory (non-fatal)**

After the report file is written, ingest it into the Pinecone memory layer so
future sessions, the routine sweeper (`/trade routine`), and the recall layer
(`/trade recall`) can find it. This is **non-fatal**: memory failures
(missing API key, network blip, schema drift) must NEVER abort the analysis
or surface as an error to the user — the report file on disk is the
...
upsert still completes. If the env var is set, the ingest emits a structured
`[archive-todo]` JSON line on stderr — the calling skill (or the routine, if
that's the context) may grep for it and perform the Drive upload via the
`mcp__claude_ai_Google_Drive__*` tools (`search_files` →
`create_file` folder → `create_file` document). For direct
`/trade analyze <TICKER>` invocations from a user, the `[archive-todo]` line
is best surfaced as a one-line confirmation in Step 5 ("Drive archive queued
for <TICKER>/") and otherwise left for the routine to consume.

**Step 5 — Confirm Output**

After writing the file (and attempting the non-fatal ingest), display a
summary in the terminal:
- The Trade Score, Grade, and Signal
- The file path where the report was saved
- A one-line note on the ingest outcome (success, "memory unavailable —
  report still saved", or any `[archive-todo]` queued)
