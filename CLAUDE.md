...
- **Pinecone record schema is a public contract.** The frontmatter / metadata field names emitted by `trade_memory.py ingest` (`schema_version`, `ticker`, `company`, `report_type`, `composite_score`, `signal`, `grade`, `run_id`, `source_path`, …) are consumed by downstream readers (the trading-chatbot in `plan/trading-chatbot.md` is the reference consumer). Field renames, type changes, or enum-value removals require coordinated migration + a `schema_version` bump. Additive changes are safe. Full table + ID scheme + 6-band signal labels live in `README.md` → "Consumer Integration". Single source of truth: `scripts/trade_schemas.py`. The `OPTIONS` report_type + the options fields (`iv_rank`, `strategy_outlook`, `recommended_strategy`, `position_bias`) were added additively (no `schema_version` bump). **The schema is vendored into the proxy: after editing `scripts/trade_schemas.py` you MUST run `bash scripts/sync_proxy_schemas.sh` (which copies it to `proxy/_lib/trade_schemas.py`, enforced by the D.17 diff gate), and the proxy must redeploy (push to main → Vercel) before the *cloud* routine can ingest new fields/types — `proxy/_lib/validate.py` re-validates with `extra="forbid"`.**
